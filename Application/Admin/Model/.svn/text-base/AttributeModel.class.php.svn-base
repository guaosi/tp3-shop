...
class AttributeModel extends Model 
{
	...
	public function search($pageSize = 20)
	{
        ...
        $data['count']=$count;
		/************************************** 取数据 ******************************************/
		$data['data'] = $this->alias('a')->field('a.*,b.type_name')->join('left join p39_type as b on a.type_id=b.id')->where($where)->group('a.id')->limit($page->firstRow.','.$page->listRows)->select();
		return $data;
	}
	// 添加前
	...
	{
	    //将，号统一转为英文的逗号
        $data['attr_option_values']=str_replace('，',',',I('post.attr_option_values'));
	}
	// 修改前
	...
	/************************************ 其他方法 ********************************************/
    //获得一个类型的属性
    public function getGoodsAttr($type_id){
